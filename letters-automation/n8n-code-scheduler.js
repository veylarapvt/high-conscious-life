// ============================================================================
// n8n Code node — WEEKLY SCHEDULER  ("Prepare Sends")
// Input: all rows from the subscribers sheet.
// Output: one item per subscriber whose next_send_date === today (IST) & active,
//         with { to, subject, html, row } (row = updated values to write back).
// Self-contained: the letter bank + email template live right here.
// ============================================================================
...
function pickId(count) { let n = (Number(count) || 0) + 1; if (n > 12) n = 5 + ((n - 5) % 8); return n; }
function esc(s) { return String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); }

function render(sub) {
  const today = istToday();
  const p = today.split("-").map(Number);
  const future_year = String(p[0] + 5);
  const future_date = MONTHS[p[1] - 1] + " " + p[2] + ", " + (p[0] + 5);
  const f = {
    first_name: (sub.first_name || "friend").toString().trim(),
    dream_answer: (sub.dream_answer || "the life you pictured").toString().trim(),
    future_date: future_date,
    future_year: future_year
  };
  const merge = s => String(s).replace(/{{\s*(\w+)\s*}}/g, (m, k) => (f[k] !== undefined ? f[k] : m));
  const id = pickId(sub.letter_count);
  const L = LETTERS.find(x => x.id === id) || LETTERS[0];
  const subject = merge(L.subject);
  ...
  return { id: id, subject: subject, html: html };
}

// ---- SCHEDULER: pick everyone due today (IST), render, prepare updated row ----
const today = istToday();
...
for (const sub of rows) {
  ...
  if (String(sub.next_send_date || "").trim() !== today) continue;

  const r = render(sub);
  const randDays = 5 + Math.floor(Math.random() * 5); // 5..9
  ...
  out.push({
    json: {
      ...
      row: {
        ...
        email: sub.email,
        dream_answer: sub.dream_answer,
        signup_date: sub.signup_date,
        letter_count: (Number(sub.letter_count) || 0) + 1,
        last_sent_date: today,
        next_send_date: addDays(today, randDays),
        status: "active"
      }
    }
  });
