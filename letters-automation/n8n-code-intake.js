// ============================================================================
// n8n Code node — INTAKE  ("Build Letter 1 + Row")
// Runs on a new subscriber. Renders Letter #1 and prepares the row to store.
// Self-contained: the letter bank + email template live right here.
// To edit letters later, edit the LETTERS array below (mirror of letters.json).
// ============================================================================
...
function render(sub) {
  ...
  return { id: id, subject: subject, html: html, today: today };
}

// ---- INTAKE: send Letter #1, then build the row to store ----
const sub = $json;
const r = render({ first_name: sub.first_name, dream_answer: sub.dream_answer, letter_count: 0 });
...
return [{
  json: {
    ...
    row: {
      ...
      signup_date: r.today,
      letter_count: 1,
      last_sent_date: r.today,
      next_send_date: addDays(r.today, 2),
      status: "active"
    }
  }
}];
